...
import static queries.Q_ScriptEditCall.language;
...
public final class Q_ScriptEdit extends JFrame {
    ...
    String curDir, noRowUpd, noRowDel, noRowSav, file, script, exists, wasDelLoc, wasCreated, wasDel, notInDir, 
    ...
    public Q_ScriptEdit(Q_ScriptEditCall scriptEditCall,
        ...
        // -------------
        if (!isError) {
            retCode = createWindow(scriptEditCall);
        }
     ...
     * @return 
     */
    protected String[] createWindow(Q_ScriptEditCall scriptEditCall) {
        ...
        columnListButton.addActionListener(ae -> {
            ...
            retCode = columnLists.createWindow();
            ///System.out.println("retCode[0]: "+retCode[0]);
            ///System.out.println("retCode[1]: "+retCode[1]);
            if ("ERROR".equals(retCode[0])) {
                scriptEditCall.scriptListMsgLabel.setText(retCode[1]);
                scriptEditCall.scriptListMsgLabel.setForeground(new Color(190, 60, 50));
                Q_ScriptEditCall.scriptListMsgPanel.add(scriptEditCall.scriptListMsgLabel);
        ...
        createFindWindow = new CreateFindWindow();
        ...
        runScriptButton.addActionListener(ae -> {
            ...
            rewritePcFile();  // Rewrite the changed file
            ...
            scriptRunCall.retCode = scriptRunCall.performScript(scriptName, scriptDescription);
            // Handle messages
            JLabel msgLabel = new JLabel();
            Q_ScriptEditCall.scriptListMsgPanel.removeAll();
            scriptEditCall.repaint(); 
            if (scriptRunCall.retCode[0].contains("ERROR")) {
                msgLabel.setText(scriptRunCall.retCode[1]);
                Q_ScriptEditCall.scriptListMsgPanel.add(msgLabel);
                msgLabel.setForeground(scriptEditCall.DIM_RED); // red
                scriptEditCall.setVisible(true); // This message made visible in Q_ScriptEditCall window
                scriptEditCall.repaint();
                scriptEditCall.toFront(); // Brings editor window to the front and may make it the focused window
            } else {
                msgLabel.setText(scriptRunCall.retCode[1]);
                Q_ScriptEditCall.scriptListMsgPanel.add(msgLabel);
                msgLabel.setForeground(scriptEditCall.DIM_BLUE); // blue
                scriptEditCall.setVisible(true); // This message made visible in Q_ScriptEditCall window
                scriptEditCall.repaint();
                scriptEditCall.toFront(); // Brings editor window to the front and may make it the focused window
            }
    ...
    protected void highlightBlocks(JTextArea textArea) {
        ...
        // Special comments
        blkStmts.put("--;", blockYellowLighter);
        // Query
        blkStmts.put("SELECT", blockBlueLighter);
        blkStmts.put("FROM", blockBlueLighter);
        ...
        blkStmts.put("SET", blockGrayLighter);
        blkStmts.put("SCHEMA", blockGrayLighter);
        ...
        String textToHighlight = textArea.getText().toUpperCase();
        ...
        try {
            ...
            while ( // "textToHighlight" is not empty
                    ...
                    // and at least one new-line character exists at the end of line.
                    startOfLine > -1
                ...
                if (endOfLine - startOfLine > 0) {
                    ...
                    for (String blockStmt : keyset) { // for all keys  
                        ...
                        // All block statements (SELECT, FROM, ...) in the line are highlighted.
                        while (startOfBlockStmt >= startOfLine && startOfBlockStmt <= endOfLine - blockStmt.length()) {
                            posDoubleDash = textToNl.indexOf("--", startOfLine);
                            specialCommentPos = textToNl.indexOf("--;", startOfLine); // Trigraph - special SQL line comment
                            if (specialCommentPos == -1 
                                    && posDoubleDash == -1) {
                                blockHighlighter.addHighlight(startOfBlockStmt, endOfBlockStmt, blkStmts.get(blockStmt));
                            } else if (specialCommentPos == -1 
                                    && posDoubleDash > -1 
                                    && endOfBlockStmt < posDoubleDash) {
                                blockHighlighter.addHighlight(startOfBlockStmt, endOfBlockStmt, blkStmts.get(blockStmt));
                            } else if (specialCommentPos > 0  && startOfBlockStmt == specialCommentPos
                                    || specialCommentPos > 0  && "?".equals(blockStmt)) {
                                blockHighlighter.addHighlight(startOfBlockStmt, endOfBlockStmt, blkStmts.get(blockStmt));
                            }
