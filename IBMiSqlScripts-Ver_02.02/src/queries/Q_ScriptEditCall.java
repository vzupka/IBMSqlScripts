...
public final class Q_ScriptEditCall extends JFrame {
    ...
    String curDir, noRowUpd, noRowDel, noRowSav, file, script, exists, scriptSuccess, wasDelLoc, wasCreated, wasDel, notInDir, 
    ...
    public Q_ScriptEditCall() {
        ...
        scriptListExportButton.addActionListener(a -> {
            ...
            if (rowIndexList != null) {
                if (scriptListIndexSel >= 0) {
                    ...
                    if (msg.startsWith("!")) { // probably connection timeout
                        scriptListMsgLabel.setForeground(DIM_RED); // red
                    } else {
                        scriptListMsgLabel.setForeground(DIM_BLUE); // blue
                    }
        ...
        scriptListImportButton.addActionListener(a -> {
            ...
            // If the list is not empty - call method "importScript()"
            if (rowIndexList != null) {
                if (scriptListIndexSel >= 0) {  // A row is selected
                    // scriptListMsgLabel.setText("");
                    scriptListMsgPanel.add(scriptListMsgLabel);
                    scriptListIndexSel = rowIndexList.getLeadSelectionIndex();
                ...
                } // No row is selected - call method "showPrompt()" - Prompt for script name to import
                else {
                    // Script file name is not known - the user must enter a name in a dialog.
                    Q_PromptImportScript prompt = new Q_PromptImportScript();
                    prompt.showPrompt();
                }
            } // If the list is empty - call method "showPrompt()" - Prompt for script name to import
            else {
                Q_PromptImportScript prompt = new Q_PromptImportScript();
                prompt.showPrompt();
            }
    ...
    /**
     * Read all keys (script names) and values (script data) from the tree map "scriptNames"
     * and place them into array "records" to fill the JTable in the script list
     *
    ...
    /**
     * Reads script files from directory "scriptfiles" 
     * and puts data to tree map "scriptNames" (String, StringBuilder)
     *
     * @return 
     */
