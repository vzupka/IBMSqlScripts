...
import static queries.Q_ImportAllFromAS400.host;
import static queries.Q_ImportAllFromAS400.row;
...
public class Q_ScriptRun {
    ...
    /**
     * Method "runScript()" performs the SQL statement - called from method "performSqlStatement()"
     *
     * @param connection Connection object for DB2 database
     ...
     *
     */
    public String[] runScript(Connection connection, String scriptName, String stmtDescription,
        ...
        System.out.println("method runScript was called from method 'performSqlStatement' in class Q_ScriptRunCall.");
        ...
        try {
            nbrColSpaces = Integer.valueOf(colSepSpaces);
        ...
        try {    
            if (!markerArrayList.isEmpty()) {
                ...
                pstmt = Q_ConnectDB.connection.prepareCall(statement, ResultSet.TYPE_SCROLL_SENSITIVE, ResultSet.CONCUR_READ_ONLY);
                ...
                for (int idx = 0; idx < markerArrayList.size(); idx++) {
                    ...
                    switch (markerValues[MARKER_TYPE_INDEX].toUpperCase()) {
                        case "DEC":
                        case "DECIMAL":
                            pstmt.setBigDecimal(Integer.parseInt(markerPosition),
                                    new BigDecimal(markerValues[MARKER_VALUE_INDEX]));
                            break;
                        case "INT":
                        case "INTEGER":
                            pstmt.setInt(Integer.parseInt(markerPosition), Integer.parseInt(markerValues[MARKER_VALUE_INDEX]));
                            break;
                        case "DATE":
                            pstmt.setDate(Integer.parseInt(markerPosition),
                                    Date.valueOf(markerValues[MARKER_VALUE_INDEX]));
                            break;
                        case "TIME":
                            pstmt.setTime(Integer.parseInt(markerPosition),
                                    Time.valueOf(markerValues[MARKER_VALUE_INDEX]));
                            break;
                        case "TIMESTAMP":
                            pstmt.setTimestamp(Integer.parseInt(markerPosition),
                        ...
                        default:
                            ...
                            pstmt.setObject(Integer.parseInt(markerPosition), markerValues[MARKER_VALUE_INDEX]);
                    }
                }
                ...
                stmt = Q_ConnectDB.connection.createStatement(ResultSet.TYPE_SCROLL_SENSITIVE, ResultSet.CONCUR_READ_ONLY);
            ...
            if (query_or_update.equals("QUERY")) {
                ...
                if (rs.first()) {
                    ...
                    if (!totalArrayList.isEmpty()) {
                        ...
                        try {
                            spaceB = Integer.valueOf(stringSpaces);
                        } catch (Exception e) {
                            spaceB = 0;
                        }
                        ...
                        try {
                            spaceA = Integer.valueOf(stringSpaces);
                        } catch (Exception e) {
                            spaceA = 0;
                        }
                        // Null mark
                        ...
                        if (totalValues.length > 3) {
                            ...
                            try {
                                Integer newNbrColSpaces = Integer.valueOf(stringSpaces);
                                // Create a character array with this number of spaces
...
//                headerArrayList.set(hdr, headerValues[hdr]);
    ...
    protected String editCurrentRowColumnValue(int index, ArrayList<Integer> colTypes, ArrayList<String> colNames, ArrayList<Integer> colIndexes) throws SQLException {
        ...
        if (rs.getObject(colIndexes.get(index)) == null) {
            ...
            // Decimal numbers are printed with or without the pattern and are localized
    ...
    protected String editNumericDecimal(BigDecimal rs_DecimalValue, String colName) {
        ...
        if (!patternArrayList.isEmpty()) {
            ...
            for (int in = 0; in < patternArrayList.size(); in++) {
                // If the current column name matches a name [1] in pattern definition line,
